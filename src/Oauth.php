...
class Oauth {
	...
	protected $appscret = null;
	public function __construct($redirect_uri){
		$this->appid = config('wx_appid');
		$this->appscret = config('wx_appscret');
		if(empty($this->appid)){
			throw new E('weixin appid need config');
		}
		if(empty($this->appscret)){
			throw new E('weixin appscret need config');
		}
	...
	public function getCode(){
	    ...
	    $protocol = (!empty($_SERVER['HTTPS']) && $_SERVER['HTTPS'] !== 'off' || $_SERVER['SERVER_PORT'] == 443) ? "https://" : "http://";
	    // $Notify_url = "$protocol$_SERVER[HTTP_HOST]".$callcall_uri;
		$redirect_uri = urlencode("$protocol$_SERVER[HTTP_HOST]".$this->redirect_uri);
		$state = I('get.url');
		// $is_scope = max(0,$_GET['is_scope']);
	...
	public function getUserInfo(){
        $appid = $this->appid;//"wx3a5aac7161b28013"; //公众号的唯一标识
        $secret = $this->appscret;//C('APPSECRET');//"d4624c36b6795d1d99dcf0547af5443d";  //公众号的appsecret
	        // trace($appid.'/////'.$secret,'=====================access_token==========================','DEBUG');
        $code = $_GET["code"];  //第一步获取的code参数
